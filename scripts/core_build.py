...
def get_compilers(name, is_mcu=True):
    if is_mcu:
        if any(substring in name for substring in ["SAM", "STM", "TM4C", "MK", "MC", "R7FA", "K32L", "KW45", "M0", "M2", "M4", "NUC", "NANO", "MINI", "M23", "XMC", "CY8C", "MSP"]):
        ...
        with open(file, 'r') as f:
            ...
            for line in f:
                if isGCC and 'list(APPEND local_list_include' in line:
                    ...
                    if 'doc_ds' in systemPath or ('sam' in systemPath and re.search('^(at)?sam.+$', file_name)) or 'nuvoton' in systemPath or 'pic32' in systemPath or 'infineon' in systemPath or ('ti' in systemPath and 'mspm' in file_name):
